...
- Deferred XP for non-admin threads: DONE. For threads not created by an admin (or with `xpAward: "onClose"`), `publishForumPost` accumulates per-pokemon XP into `thread.pendingXp` instead of applying it; admin/staff threads apply immediately (gated by the thread's `createdByAdmin`). At close, `ThreadRewards.tsx` seeds an editable per-pokemon table from `pendingXp` and `finalizeThreadRewards` commits the reviewed `pokemonXp` to `owned_pokemons`, gated by the `GiveItems`/`ReviewRewards` capabilities.
