...
**What's broken (beyond cost):** posts store `displayName` only (no uid), which blocks
...
9. **Unify bookmarks** into `users/{uid}/bookmarks/{threadId}` docs `{forum, threadId, title, notify, addedAt}`. These replace both the regex-string `myBookmarks` array and the orphaned `bookmarks/Main-Forum` map. One shared `useQuery(['bookmarks', uid])`; one-off migration of old strings.
